...
public class JfreeChartScatter {
	...
	/** The data. */
    private double[][] data = new double[2][200];
     ...
     * 调用JfreeChart绘制散点图
     */
	public void Scatter(ArrayList<ArrayList<Point>> cluster)
	{
		...
		for(int i = 0; i < cluster.size(); i++)
		{
			...
			{
				this.data[0][j] = cluster.get(i).get(j).getX();
				this.data[1][j] = cluster.get(i).get(j).getY();
			}
		...
		JFreeChart jfree = ChartFactory.createScatterPlot("DBSCAN", "X", "Y", xyDataset);
		...
		renderer.setSeriesOutlineStroke(3, new BasicStroke(0.05f));
